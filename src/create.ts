...
export const getBondingCurveConfigs = ({
  ...
  if (!mint && !mintBump) {
    throw new Error("mint or mintBump must be provided");
  }

  const mintPublicKey = mint ? mint : PublicKey.findProgramAddressSync(
    [ammConfig.amm.toBuffer(), mintBump!.toBuffer()],
    PROGRAM_ADDRESS
  )[0];
  ...
  const mintTokenKey = mintPublicKey;
  ...
  return {
    ...
    pool: poolKey,
    lock: lockKey,
    lockMaster: ammConfig.lockMaster,
    admin: ammConfig.admin,
    migrator: ammConfig.migrator,
    lockVault,
    poolAuthority,
    poolTokenAccount,
    poolLockAuthority,
